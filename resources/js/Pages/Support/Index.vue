<template>
    ...
    <MainLayout>
        ...
        <section class="relative pt-32 pb-20 px-4 sm:px-6 lg:px-8">
            <div class="max-w-7xl mx-auto text-center">
                <div class="inline-block mb-4">
                    <span class="text-sm font-semibold text-audiogold-600 tracking-wider uppercase">{{ supportSettings.hero_badge }}</span>
                </div>
                <h1 class="text-5xl md:text-6xl font-bold mb-6">
                    <span class="bg-gradient-to-r from-audiogold-600 to-audiogold-800 bg-clip-text text-transparent">
                        {{ supportSettings.hero_title }}
                    </span>
                </h1>
                <p class="text-xl text-gray-600 max-w-3xl mx-auto mb-4">
                    {{ supportSettings.hero_subtitle }}
                </p>
        ...
        <section class="py-12 px-4 sm:px-6 lg:px-8">
            <div class="max-w-7xl mx-auto">
                <div class="grid md:grid-cols-4 gap-6">
                    <a v-for="(option, index) in supportSettings.quick_support_options" :key="index"
                       :href="option.link"
                       :target="option.link.startsWith('http') ? '_blank' : '_self'"
                       class="group backdrop-blur-md bg-white/60 rounded-2xl p-6 shadow-xl border border-white/50 hover:shadow-2xl hover:bg-white/80 transition-all duration-300 transform hover:-translate-y-2 cursor-pointer">
                        <div :class="[
                            'w-14 h-14 rounded-xl flex items-center justify-center mb-4 group-hover:scale-110 group-hover:rotate-6 transition-all duration-300 shadow-lg',
                            `bg-gradient-to-br ${option.gradient}`
                        ]">
                            <component :is="getIconComponent(option.icon)" class="w-7 h-7 text-white" />
                        </div>
                        <h3 class="font-bold text-gray-800 mb-2">{{ option.title }}</h3>
        ...
        <!-- Self-Help Resources -->
        <section id="self-help" class="py-12 px-4 sm:px-6 lg:px-8">
            <div class="max-w-7xl mx-auto">
                <div class="text-center mb-12">
                    <h2 class="text-4xl font-bold mb-4">
                        <span class="bg-gradient-to-r from-audiogold-600 to-audiogold-800 bg-clip-text text-transparent">
                            {{ supportSettings.self_help_title }}
                        </span>
                    </h2>
                    <p class="text-gray-600">{{ supportSettings.self_help_subtitle }}</p>
                </div>

                <div class="grid md:grid-cols-2 gap-8">
                    <div v-for="(resource, index) in supportSettings.self_help_resources" :key="index"
                         class="group backdrop-blur-md bg-white/60 rounded-3xl p-8 shadow-2xl border border-white/50 hover:shadow-3xl hover:bg-white/80 transition-all duration-500 transform hover:-translate-y-2">
                        <div class="flex items-start space-x-4">
                            <div class="flex-shrink-0">
                                <div :class="[
                                    'w-16 h-16 rounded-2xl flex items-center justify-center group-hover:scale-110 group-hover:rotate-6 transition-all duration-300 shadow-lg',
                            ...
                            <div class="flex-1">
                                <h3 class="text-2xl font-bold text-gray-800 mb-3">{{ resource.title }}</h3>
                                <p class="text-gray-700 mb-4">{{ resource.description }}</p>
                                <a :href="resource.link" class="inline-flex items-center gap-2 text-audiogold-600 font-semibold hover:text-audiogold-700 transition-colors duration-200">
                                    {{ resource.link_text }}
        ...
        <section class="py-12 px-4 sm:px-6 lg:px-8">
            <div class="max-w-7xl mx-auto">
                <div class="text-center mb-12">
                    <h2 class="text-4xl font-bold mb-4">
                        <span class="bg-gradient-to-r from-audiogold-600 to-audiogold-800 bg-clip-text text-transparent">
                            {{ supportSettings.product_support_title }}
                        </span>
                    </h2>
                    <p class="text-gray-600">{{ supportSettings.product_support_subtitle }}</p>
                </div>

                <div class="grid md:grid-cols-3 gap-6">
                    <div v-for="(card, index) in supportSettings.product_support_cards" :key="index"
                        ...
                        <div :class="[
                            'w-12 h-12 rounded-xl flex items-center justify-center mb-4 group-hover:scale-110 transition-all duration-300 shadow-lg',
                        ...
                        <a :href="card.link" class="text-audiogold-600 font-semibold text-sm hover:text-audiogold-700">{{ card.link_text }}</a>
                    </div>
                </div>
            </div>
        </section>
        ...
        <section class="py-12 px-4 sm:px-6 lg:px-8">
            <div class="max-w-7xl mx-auto">
                <div class="backdrop-blur-md bg-gradient-to-br from-audiogold-500 to-audiogold-700 rounded-3xl p-8 md:p-12 shadow-2xl border border-audiogold-400/50 relative overflow-hidden">
                    <div class="absolute top-0 right-0 w-64 h-64 bg-white/10 rounded-full blur-3xl"></div>
                    <div class="relative z-10">
                        <div class="grid md:grid-cols-2 gap-8 items-center">
                            <div>
                                <h2 class="text-3xl md:text-4xl font-bold text-white mb-4">{{ supportSettings.video_title }}</h2>
                                <p class="text-white/90 mb-6">{{ supportSettings.video_subtitle }}</p>
                                <a :href="supportSettings.video_button_link" class="inline-flex items-center gap-2 px-6 py-3 bg-white text-audiogold-700 rounded-xl font-semibold hover:bg-gray-100 transition-all duration-300 shadow-lg">
                                    {{ supportSettings.video_button_text }}
                            ...
                            <div class="flex justify-center">
                                <div class="w-32 h-32 bg-white/20 rounded-full flex items-center justify-center backdrop-blur-sm animate-pulse-slow">
        ...
        <!-- Warranty and Repairs + Order Support -->
        <section class="py-12 px-4 sm:px-6 lg:px-8">
            <div class="max-w-7xl mx-auto">
                <div class="grid md:grid-cols-2 gap-8">
                    ...
                    <div class="backdrop-blur-md bg-white/60 rounded-3xl p-8 shadow-2xl border border-white/50 hover:shadow-3xl transition-all duration-500">
                        <div class="flex items-start gap-4 mb-6">
                            ...
                            <div>
                                <h2 class="text-2xl font-bold text-gray-800 mb-2">{{ supportSettings.warranty_title }}</h2>
                                <p class="text-gray-600">{{ supportSettings.warranty_subtitle }}</p>
                            </div>
                        </div>
                        <ul class="space-y-3">
                            <li v-for="(feature, index) in supportSettings.warranty_features" :key="index" class="flex items-start gap-2">
                                <svg class="w-5 h-5 text-audiogold-600 mt-0.5 flex-shrink-0" fill="currentColor" viewBox="0 0 24 24">
                                    <path d="M9 16.2L4.8 12l-1.4 1.4L9 19 21 7l-1.4-1.4L9 16.2z"/>
                                </svg>
                                <span class="text-gray-700">{{ feature }}</span>
                            </li>
                        </ul>
                        <a href="#contact-form" class="inline-flex items-center gap-2 mt-6 text-audiogold-600 font-semibold hover:text-audiogold-700 transition-colors duration-200">
                            {{ supportSettings.warranty_link_text }}
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/>
                            </svg>
                        </a>
                    ...
                    <div class="backdrop-blur-md bg-white/60 rounded-3xl p-8 shadow-2xl border border-white/50 hover:shadow-3xl transition-all duration-500">
                        <div class="flex items-start gap-4 mb-6">
                            ...
                            <div>
                                <h2 class="text-2xl font-bold text-gray-800 mb-2">{{ supportSettings.order_title }}</h2>
                                <p class="text-gray-600">{{ supportSettings.order_subtitle }}</p>
                            </div>
                        </div>
                        <ul class="space-y-3">
                            <li v-for="(feature, index) in supportSettings.order_features" :key="index" class="flex items-start gap-2">
                                <svg class="w-5 h-5 text-audiogold-600 mt-0.5 flex-shrink-0" fill="currentColor" viewBox="0 0 24 24">
                                    <path d="M9 16.2L4.8 12l-1.4 1.4L9 19 21 7l-1.4-1.4L9 16.2z"/>
                                </svg>
                                <span class="text-gray-700">{{ feature }}</span>
                        ...
                        <a href="#contact-form" class="inline-flex items-center gap-2 mt-6 text-audiogold-600 font-semibold hover:text-audiogold-700 transition-colors duration-200">
                            {{ supportSettings.order_link_text }}
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/>
                            </svg>
                        </a>
        ...
        <section id="contact-form" class="py-12 px-4 sm:px-6 lg:px-8">
            <div class="max-w-7xl mx-auto">
                ...
                <div class="text-center mb-12">
                    <h2 class="text-4xl font-bold mb-4">
                        <span class="bg-gradient-to-r from-audiogold-600 to-audiogold-800 bg-clip-text text-transparent">
                            {{ supportSettings.contact_title }}
                        </span>
                    </h2>
                    <p class="text-gray-600">{{ supportSettings.contact_subtitle }}</p>
                </div>

                <div class="grid lg:grid-cols-2 gap-12">
                    <!-- Contact Form -->
                    <div class="backdrop-blur-md bg-white/60 rounded-3xl p-8 md:p-10 shadow-2xl border border-white/50 hover:shadow-3xl hover:bg-white/70 transition-all duration-500">
                        <form @submit.prevent="submitForm" class="space-y-6">
                            ...
                            <div>
                                ...
                                ></textarea>
                                <p class="text-sm text-gray-500 mt-2">{{ form.message.length }} / 180</p>
                            </div>
                            ...
                            <div class="space-y-5">
                                <div class="flex items-start space-x-4 p-3 rounded-xl hover:bg-white/60 transition-all duration-300">
                                    ...
                                    <div>
                                        ...
                                        <a :href="`mailto:${supportSettings.email}`" class="text-audiogold-600 hover:text-audiogold-700 break-all">
                                ...
                                <div class="flex items-start space-x-4 p-3 rounded-xl hover:bg-white/60 transition-all duration-300">
                                    ...
                                    <div>
                                        ...
                                        <a :href="`tel:${supportSettings.phone?.replace(/\s/g, '')}`" class="text-audiogold-600 hover:text-audiogold-700">
                                ...
                                <div class="flex items-start space-x-4 p-3 rounded-xl hover:bg-white/60 transition-all duration-300">
                                    ...
                                    <div>
                                        ...
                                        <a :href="`https://wa.me/${supportSettings.whatsapp?.replace(/[^0-9]/g, '')}`" target="_blank" class="text-audiogold-600 hover:text-audiogold-700">
                                ...
                                <div class="flex items-start space-x-4 p-3 rounded-xl hover:bg-white/60 transition-all duration-300">
                                    ...
                                    <div>
                                        ...
                                        <p class="text-gray-600">{{ supportSettings.timing_days }}<br>{{ supportSettings.timing_hours }}</p>
                                    </div>
                                </div>
                                ...
                                <div class="flex items-start space-x-4 p-3 rounded-xl hover:bg-white/60 transition-all duration-300">
                                    ...
                                    <div>
                                        ...
                                        <p class="text-gray-600 whitespace-pre-line">{{ supportSettings.address }}</p>
                                    </div>
                                </div>
                            </div>
                        </div>
...
const getIconComponent = (iconName) => {
    const icons = {
        email: {
            render() {
                return h('svg', { fill: 'currentColor', viewBox: '0 0 24 24', class: 'w-7 h-7 text-white' }, [
        ...
        whatsapp: {
            render() {
                return h('svg', { fill: 'currentColor', viewBox: '0 0 24 24', class: 'w-7 h-7 text-white' }, [
                    h('path', { d: 'M12.04 2c-5.46 0-9.91 4.45-9.91 9.91 0 1.75.46 3.45 1.32 4.95L2.05 22l5.25-1.38c1.45.79 3.08 1.21 4.74 1.21 5.46 0 9.91-4.45 9.91-9.91 0-2.65-1.03-5.14-2.9-7.01C17.18 3.03 14.69 2 12.04 2m.01 1.67c2.2 0 4.26.86 5.82 2.42a8.225 8.225 0 0 1 2.41 5.83c0 4.54-3.7 8.23-8.24 8.23-1.48 0-2.93-.39-4.19-1.15l-.3-.17-3.12.82.83-3.04-.2-.32a8.188 8.188 0 0 1-1.26-4.38c.01-4.54 3.7-8.24 8.25-8.24M8.53 7.33c-.16 0-.43.06-.66.31-.22.25-.87.85-.87 2.07 0 1.22.89 2.39 1 2.56.14.17 1.76 2.67 4.25 3.73.59.27 1.05.42 1.41.53.59.19 1.13.16 1.56.1.48-.07 1.46-.6 1.67-1.18.21-.58.21-1.07.15-1.18-.07-.1-.23-.16-.48-.27-.25-.14-1.47-.74-1.69-.82-.23-.08-.37-.12-.56.12-.16.25-.64.81-.78.97-.15.17-.29.19-.53.07-.26-.13-1.06-.39-2-1.23-.74-.66-1.23-1.47-1.38-1.72-.14-.25-.02-.39.11-.5.11-.11.25-.29.37-.44.13-.14.17-.25.25-.41.08-.17.04-.31-.02-.43-.06-.11-.56-1.35-.77-1.84-.2-.48-.4-.42-.56-.43-.14 0-.3-.01-.47-.01z' })
                ]);
            }
        },
        info: {
