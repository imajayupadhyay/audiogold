<template>
    <section class="py-20 px-4 sm:px-6 lg:px-8 relative overflow-hidden bg-white">
        <div class="max-w-7xl mx-auto relative z-10">
            ...
            <div class="grid md:grid-cols-3 gap-8 mb-12">
                <div
                    v-for="(benefit, index) in settings.dealer_benefits"
                    :key="index"
                    class="group relative bg-white rounded-3xl p-8 shadow-xl border border-gray-100 hover:shadow-2xl transition-all duration-500 overflow-hidden"
                >
                    <div :class="['absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-500 rounded-3xl', `bg-gradient-to-br ${benefit.bg_gradient}`]"></div>

                    <div class="relative z-10">
                        <div :class="['rounded-2xl p-4 w-16 h-16 flex items-center justify-center mb-6 transform group-hover:scale-110 group-hover:rotate-3 transition-all duration-500 shadow-lg', `bg-gradient-to-br ${benefit.gradient}`]">
                            <svg v-if="index === 0" class="w-8 h-8 text-white" fill="currentColor" viewBox="0 0 24 24">
                                <path d="M11.8 10.9c-2.27-.59-3-1.2-3-2.15 0-1.09 1.01-1.85 2.7-1.85 1.78 0 2.44.85 2.5 2.1h2.21c-.07-1.72-1.12-3.3-3.21-3.81V3h-3v2.16c-1.94.42-3.5 1.68-3.5 3.61 0 2.31 1.91 3.46 4.7 4.13 2.5.6 3 1.48 3 2.41 0 .69-.49 1.79-2.7 1.79-2.06 0-2.87-.92-2.98-2.1h-2.2c.12 2.19 1.76 3.42 3.68 3.83V21h3v-2.15c1.95-.37 3.5-1.5 3.5-3.55 0-2.84-2.43-3.81-4.7-4.4z"/>
                            </svg>
                            <svg v-else-if="index === 1" class="w-8 h-8 text-white" fill="currentColor" viewBox="0 0 24 24">
            ...
            <!-- CTA Section -->
            <div class="relative bg-gradient-to-br from-audiogold-500 to-audiogold-700 rounded-3xl p-12 md:p-16 shadow-2xl overflow-hidden">
                ...
                <div class="relative z-10 text-center">
                    ...
                    <h3 class="text-3xl md:text-4xl font-bold text-white mb-4">
                        {{ settings.dealer_cta_title }}
                    </h3>
                    <p class="text-lg text-white/90 max-w-2xl mx-auto mb-8">
                        {{ settings.dealer_cta_subtitle }}
                    </p>

                    <div class="flex flex-col sm:flex-row gap-4 justify-center">
                        <a :href="settings.dealer_cta_primary_link"
                           class="group px-8 py-4 bg-white text-audiogold-700 rounded-full font-bold text-lg hover:bg-gray-100 transition-all duration-300 shadow-lg hover:shadow-xl transform hover:-translate-y-1 flex items-center justify-center gap-2">
                            {{ settings.dealer_cta_primary_text }}
                            <svg class="w-5 h-5 group-hover:translate-x-1 transition-transform duration-300" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 7l5 5m0 0l-5 5m5-5H6"/>
                            </svg>
                        </a>
                        <a :href="settings.dealer_cta_secondary_link"
                           target="_blank"
                           class="group px-8 py-4 bg-white/10 backdrop-blur-sm text-white rounded-full font-bold text-lg border-2 border-white/40 hover:bg-white/20 hover:border-white/60 transition-all duration-300 shadow-lg hover:shadow-xl transform hover:-translate-y-1 flex items-center justify-center gap-2">
                            {{ settings.dealer_cta_secondary_text }}
        ...
        <div class="absolute top-20 left-10 w-72 h-72 bg-gradient-to-br from-orange-200 to-amber-200 rounded-full blur-3xl opacity-20"></div>
        <div class="absolute bottom-20 right-10 w-64 h-64 bg-gradient-to-br from-green-200 to-emerald-200 rounded-full blur-3xl opacity-20"></div>
    </section>
</template>
